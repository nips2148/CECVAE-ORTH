...
def table_str_bin(result_set, row_labels, labels_long=None, binary=False):
    ...
    for i in range(len(result_set)):
        ...
        stds = [np.std(np.abs(result_set[i][c])) / np.sqrt(result_set[i][c].shape[0]) for c in cols]
...
def select_parameters(results, configs, stop_set, stop_criterion, choice_set, choice_criterion):
    ...
    # i_sel = np.nanargmin(results[stop_set][stop_criterion], 2)
    ...
    # configs = np.nanargmin(results_sel['valid']['pehe'][:, 0:5], 0)
    # valid_pehe = np.nanmin(results_sel['valid']['pehe'][:, 0:5], 0)
    # valid_ate = np.abs(results_sel['valid']['bias_ate'][:, 0:5][configs, np.arange(5)])
    # train_pehe = results_sel['train']['pehe'][:, 0:5][configs, np.arange(5)]
    # train_ate = np.abs(results_sel['train']['bias_ate'][:, 0:5][configs, np.arange(5)])
    # test_pehe = results_sel['test']['pehe'][:, 0:5][configs, np.arange(5)]
    # test_ate = np.abs(results_sel['test']['bias_ate'][:, 0:5][configs, np.arange(5)])
    # print(configs)
    # print(valid_pehe)
    # print(valid_ate)
    # print(train_pehe)
    ...
    results_best_config = {'train': {}, 'valid': {}, 'test': {}}
    best_config_indexes = np.nanargmin(results_sel[choice_set][choice_criterion], 0)
    for k in results['valid'].keys():
        # for best_config_index in best_config_indexes:
        results_best_config['train'][k] = results_sel['train'][k][best_config_indexes, np.arange(n_exp)]
        results_best_config['valid'][k] = results_sel['valid'][k][best_config_indexes, np.arange(n_exp)]
        if k in results['test']:
            results_best_config['test'][k] = results_sel['test'][k][best_config_indexes, np.arange(n_exp)]
    ...
    for k in results_best_config.keys():
        print(k)
        print("Pehe\tBias_ate\tRmse_fact\tRmse_ite\tObjective\tPehe_nn")
        metrics = ['pehe','bias_ate','rmse_fact','rmse_fact','rmse_ite','pehe_nn']

        # for i in range(n_exp):
        #     print(
        #         "{}\t{}\t{}\t{}\t{}\t".format(results_best_config[k]['pehe'][i], results_best_config[k]['bias_ate'][i],
        #                                           results_best_config[k]['rmse_fact'][i], results_best_config[k]['rmse_ite'][i],
        #                                           results_best_config[k]['pehe_nn'][i]))
        # print("\n")

        # metric_str = ""
        # for metric in metrics:
        #     val = np.mean(np.abs(results_best_config[k][metric]))
        #     std = np.std(np.abs(results_best_config[k][metric])) / np.sqrt(results_best_config[k][metric].shape[0])
        #     metric_str += "{:.3f}±{:.3f}\t".format(val, std)
        # print(metric_str)
        # print("\n")
