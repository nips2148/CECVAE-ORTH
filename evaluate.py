...
from cvae.logger import Logger as Log
Log.VERBOSE = True

import cvae.evaluation as evaluation
...
def evaluate(config_file, overwrite=False, filters=None):
    ...
    data_train_load = load_data(data_train)
    if binary and 'mu0' not in data_train_load:
        plot_evaluation_bin(eval_results, configs, output_dir, data_train, data_test, filters)
    # elif binary:
    #     plot_evaluation_cont_twins(eval_results, configs, output_dir, data_train, data_test, filters)
    else:
