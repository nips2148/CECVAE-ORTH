...
tf.app.flags.DEFINE_float('coef_t_pred', 1.0, """coefficient of treatment prediction loss. """)
tf.app.flags.DEFINE_float('coef_y_pred', 1.0, """coefficient of outcome prediction loss. """)
...
def train(CFR, train_step, D, I_valid, D_test, logfile, i_exp):
    ...
    dict_factual = {CFR.x: D['x'][I_train, :], CFR.t: D['t'][I_train, :], CFR.y: D['yf'][I_train, :], \
                    CFR.dropout_in: 1.0, CFR.dropout_out: 1.0,CFR.p: p_treated}

    if FLAGS.val_part > 0:
        dict_valid = {CFR.x: D['x'][I_valid, :], CFR.t: D['t'][I_valid, :], CFR.y: D['yf'][I_valid, :], \
                      CFR.dropout_in: 1.0, CFR.dropout_out: 1.0,CFR.p: p_treated}

    if D['HAVE_TRUTH']:
        dict_cfactual = {CFR.x: D['x'][I_train, :], CFR.t: 1 - D['t'][I_train, :], CFR.y: D['ycf'][I_train, :], \
                         CFR.dropout_in: 1.0, CFR.dropout_out: 1.0}

    ''' Initialize TensorFlow variables '''
    CFR.sess.run(tf.global_variables_initializer())
    ...
    ''' Compute losses '''
    losses = []
    obj_loss, f_error, imb_err = CFR.sess.run([CFR.tot_loss, CFR.pred_loss, CFR.imb_dist], \
                                              feed_dict=dict_factual)

    cf_error = np.nan
    if D['HAVE_TRUTH']:
        cf_error = CFR.sess.run(CFR.pred_loss, feed_dict=dict_cfactual)

    valid_obj = np.nan;
    valid_imb = np.nan;
    valid_f_error = np.nan;
    if FLAGS.val_part > 0:
        valid_obj, valid_f_error, valid_imb = CFR.sess.run([CFR.tot_loss, CFR.pred_loss, CFR.imb_dist], \
    ...
    for i in range(FLAGS.iterations):
        ...
        if not objnan:
            CFR.sess.run(train_step, feed_dict={CFR.x: x_batch, CFR.t: t_batch,
                                                CFR.y: y_batch, CFR.dropout_in: FLAGS.dropout_in,
                                                CFR.dropout_out: FLAGS.dropout_out,
                                                CFR.p: p_treated})

        ''' Project variable selection weights '''
        if FLAGS.varsel:
            wip = simplex_project(CFR.sess.run(CFR.weights_in[0]), 1)
            CFR.sess.run(CFR.projection, feed_dict={CFR.w_proj: wip})
        ...
        if i % FLAGS.output_delay == 0 or i == FLAGS.iterations - 1:
            ...
            # rep = CFR.sess.run(CFR.h_rep_norm, feed_dict={CFR.x: D['x'], CFR.dropout_in: 1.0})
            ...
            if FLAGS.loss == 'log':
                y_pred = CFR.sess.run(CFR.output, feed_dict={CFR.x: x_batch,
                                                             CFR.t: t_batch, CFR.dropout_in: 1.0, CFR.dropout_out: 1.0})
        ...
        if (FLAGS.pred_output_delay > 0 and i % FLAGS.pred_output_delay == 0) or i == FLAGS.iterations - 1:
            ...
            y_pred_f = CFR.sess.run(CFR.output, feed_dict={CFR.x: D['x'], \
                                                           CFR.t: D['t'], CFR.dropout_in: 1.0, CFR.dropout_out: 1.0})
            y_pred_cf = CFR.sess.run(CFR.output, feed_dict={CFR.x: D['x'], \
                                                            CFR.t: 1 - D['t'], CFR.dropout_in: 1.0,
                                                            CFR.dropout_out: 1.0})
            preds_train.append(np.concatenate((y_pred_f, y_pred_cf), axis=1))

            if D_test is not None:
                y_pred_f_test = CFR.sess.run(CFR.output, feed_dict={CFR.x: D_test['x'], \
                                                                    CFR.t: D_test['t'], CFR.dropout_in: 1.0,
                                                                    CFR.dropout_out: 1.0})
                y_pred_cf_test = CFR.sess.run(CFR.output, feed_dict={CFR.x: D_test['x'], \
                                                                     CFR.t: 1 - D_test['t'], CFR.dropout_in: 1.0,
                                                                     CFR.dropout_out: 1.0})
                preds_test.append(np.concatenate((y_pred_f_test, y_pred_cf_test), axis=1))

            if FLAGS.save_rep and i_exp == 1:
                reps_i = CFR.sess.run([CFR.h_rep], feed_dict={CFR.x: D['x'], \
                                                              CFR.dropout_in: 1.0, CFR.dropout_out: 0.0})
                reps.append(reps_i)

                if D_test is not None:
                    reps_test_i = CFR.sess.run([CFR.h_rep], feed_dict={CFR.x: D_test['x'], \
                                                                       CFR.dropout_in: 1.0, CFR.dropout_out: 0.0})
...
def run(outdir):
    ...
    for i_exp in range(1, n_experiments + 1):
        ...
        losses, preds_train, preds_test, reps, reps_test = \
            train(CFR, train_step, D_exp, I_valid, \
        ...
        if FLAGS.varsel:
            if i_exp == 1:
                all_weights = CFR.sess.run(CFR.weights_in[0])
                all_beta = CFR.sess.run(CFR.weights_pred)
            else:
                all_weights = np.dstack((all_weights, CFR.sess.run(CFR.weights_in[0])))
                all_beta = np.dstack((all_beta, CFR.sess.run(CFR.weights_pred)))
